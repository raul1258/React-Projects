...
function Weather() {
  ...
  useEffect(() => {
    navigator.geolocation.getCurrentPosition((geo) => {
      ...
      fetch(
        `http://api.openweathermap.org/geo/1.0/reverse?lat=${lat}&lon=${lon}&limit=5&appid=aff6f1194660ee7643d426b59c2f2742`
      )
        .then((res) => res.json())
        .then((data) => {
          // console.log(data);
  ...
  function searchBtnHandler() {
    fetch(
      `http://api.openweathermap.org/geo/1.0/direct?q=${searchInput}&limit=5&appid=aff6f1194660ee7643d426b59c2f2742`
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    <>
      ...
      {currentCity && currentCity.name && userCoord && (
        <div className="d-flex justify-content-center">
          <iframe
            ...
            src={`https://www.google.com/maps/embed/v1/directions?key=AIzaSyDZOj-BNc1O3rbkEqY8Hm2jnDTIGewiyLg&origin=${
          ...
          <iframe
            ...
            style={{ border: 0, margin: "20px" }}
            referrerPolicy="no-referrer-when-downgrade"
            src={`https://www.google.com/maps/embed/v1/place?key=AIzaSyDZOj-BNc1O3rbkEqY8Hm2jnDTIGewiyLg&q=${
              userCoord.lat + "," + userCoord.lon
            }&maptype=satellite`}
            allowFullScreen
          ></iframe>
      ...
      />
      <button
        className="d-block btn btn-primary m-auto my-2"
